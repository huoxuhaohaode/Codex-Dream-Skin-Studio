...
- 重新设计 macOS 应用图标，使用规范的 `1024 × 1024` RGBA 母版和透明外缘，修复 Dock 黑色方底与窗口切换器显示通用占位图标的问题。
- 构建时强制校验图标尺寸和透明通道，安装后重新向 LaunchServices 注册应用，降低覆盖安装后的旧图标缓存概率。
- 侧栏改为清晰的“添加主题 / 工具”分组，状态区使用轻量语义色，减少连续白色按钮造成的视觉噪声。
- 主题卡弱化重复的高亮按钮，并增加深浅外观图标；窗口名称、默认尺寸与最小尺寸统一为主题工作室语义。

## 1.9.0 — 2026-07-18

### 新增

- 新增 6 套感官体验主题：极光绸幕、声波穹厅、黑曜茶庭、雾泉石庭、柑橘工坊与棱镜晴雨；内置主题由 26 套扩展到 32 套。
- 新主题从柔光、声学木构、茶雾、温泉水汽、柑橘静物和雨后棱镜等不同元素建立氛围，并按三深三浅覆盖日夜工作场景。

### 设计边界

- 每张背景均为本项目原创生成素材，使用 `2560 × 1440` 横向构图，不含人物、文字、品牌、代码或伪装成原生控件的界面。
- 视觉焦点集中在右侧，左侧保留低信息、低对比的内容安全区；配色强调舒适辨识和长时间使用，而非短暂的高刺激效果。

### 测试

- 完整预设校验与播种回归现在覆盖 32 套内置主题，并继续验证图像尺寸、体积、配置 ID 和注入 payload。
